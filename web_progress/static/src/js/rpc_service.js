...
function _download(options) {
    ...
    if (options.data) {
        const context = options.data.context ? JSON.parse(options.data.context) : {};
...
function registerProgressBarBLockUI() {
    ...
    if (BlockUIcomp && BlockUIcomp.props) {
        ...
        BlockUIcomp.props.bus.on("UNBLOCK", null, function () {
            legacyProgressBar.removeProgressBarFrmBlockedUI()
        });
    }
